...
import CurrentInterface from './currentInterface'
const InterfaceInputComponent: React.FC = () => {
  const [textareaValue, setTextareaValue] = useState<string>('')
  const { setCurrentInterface, savedInterfaces } = useInterfaceStore()

  const handleGenerate = (): void => {
    if (textareaValue.trim()) {
      const title = textareaValue.split(' ')[2] || `T-${Date.now()}`
      const mainData: SavedInterface = {
        id: 1,
        title,
