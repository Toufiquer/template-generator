...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.accessess || [],
        [getResponseData]
    )
    ...
    const handleSort = (key: keyof IAccesses) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IAccesses) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const tableHeaders: { key: keyof IAccesses; label: string }[] = [
    ...
    const renderActions = (item: IAccesses) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedAccesses(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedAccesses(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedAccesses(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IAccesses) => (
            <TableRow key={item._id}>
                ...
                <TableCell>{(item as any)['student']}</TableCell>
                <TableCell>{(item as any)['admin']}</TableCell>
                <TableCell>{(item as any)['moderator']}</TableCell>
                <TableCell>{(item as any)['mentor']}</TableCell>
                <TableCell>{(item as any)['instructor']}</TableCell>
                    ...
                    <TableHeader className="bg-slate-100">
                        <TableRow>
                            ...
                            {tableHeaders.map(({ key, label }) => (
                                <TableHead key={key} className="cursor-pointer" onClick={() => handleSort(key as keyof IAccesses)}>
                                    {label}{' '}
