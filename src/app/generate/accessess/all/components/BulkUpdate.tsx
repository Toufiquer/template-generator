...
import { accessessSelectorArr } from '../store/store-constant'
import { useBulkUpdateAccessesMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateAccesses, { isLoading }] = useBulkUpdateAccessesMutation()
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (accessessSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {accessessSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((access, idx) => (
                            <div
                                key={(access._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(access as any)['student'] as string || ''}
                                </span>
                                <span className="text-blue-500">{access['student'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
