...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.mvs2 || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableMvs2Keys; label: string }[] = [
    ...
    const handleSort = (key: DisplayableMvs2Keys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IMvs2) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IMvs2) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedMvs2(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedMvs2(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedMvs2(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Mvs2_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
