...
import { useAddMvs2Mutation } from '../redux/rtk-api'
import { IMvs2, defaultMvs2 } from '../store/data/data'
import { formatDuplicateKeyError, handleError, handleSuccess, isApiErrorResponse } from './utils'

const AddNextComponents: React.FC = () => {
    const { toggleAddModal, isAddModalOpen, setMvs2 } = useMvs2Store()
    const [addMvs2, { isLoading }] = useAddMvs2Mutation()
    const [newMv2, setNewMv2] = useState<IMvs2>(defaultMvs2)

    const handleFieldChange = (name: string, value: unknown) => {
        setNewMv2(prev => ({ ...prev, [name]: value }));
    };

    const handleAddMv2 = async () => {
        try {
            const { _id, ...updateData } = newMv2
            const addedMv2 = await addMvs2(updateData).unwrap()
            setMvs2([addedMv2]); // Example: update store, you might need a different strategy
            toggleAddModal(false)
            setNewMv2(defaultMvs2)
    ...
    return (
        <Dialog open={isAddModalOpen} onOpenChange={toggleAddModal}>
            <DialogContent className="sm:max-w-[625px]">
                <DialogHeader>
                    <DialogTitle>Add New Mv2</DialogTitle>
                </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid grid-cols-4 items-center gap-4 pr-1">
                            ...
                            <div className="col-span-3">
                                <InputFieldForString id="title" placeholder="Title" value={newMv2['title']} onChange={(value) => handleFieldChange('title', value as string)} />
                            </div>
                        </div>
                    </div>
                </ScrollArea>
                ...
                <DialogFooter>
                    ...
                    <Button
                        disabled={isLoading}
                        onClick={handleAddMv2}
                    >
                        {isLoading ? 'Adding...' : 'Add Mv2'}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
