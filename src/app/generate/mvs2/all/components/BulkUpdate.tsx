...
import { mvs2SelectorArr } from '../store/store-constant'
import { useBulkUpdateMvs2Mutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateMvs2, { isLoading }] = useBulkUpdateMvs2Mutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateMvs2(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit mvs2:', error)
            ...
            <DialogContent>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {mvs2SelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((mv2, idx) => (
                            <div
                                key={(mv2._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(mv2)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{mv2['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
