...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const url = `${baseUrl}/generate/mvs2/all/api/v1?page=1&limit=10`;
    ...
    if (loading) {
        return <div className="text-center p-8">Loading Mvs2...</div>
    }
    ...
    return (
        <main className="w-full flex flex-col gap-4 p-1 md:p-4">
            <h1 className="text-2xl font-bold">All Mvs2</h1>
            {data.length > 0 ? (
                <div className="flex flex-col gap-2">
                    {data.map((item) => (
                        <div key={item._id}>
                            <CustomLink
                                name={item.title}
                                url={`/dashboard/mvs2/client-view/details/${item._id}`}
                            />
                        </div>
                    ))}
                </div>
