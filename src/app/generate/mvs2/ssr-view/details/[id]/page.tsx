...
const DataDetails = ({ data }: { data: IMv2 }) => {
    return (
...
const getDataById = async (id: string): Promise<ApiResponse> => {
    ...
    const backendUrl = `${baseUrl}/generate/mvs2/all/api/v1?id=${id}`
    ...
    try {
        ...
        console.error('Failed to fetch Mv2:', error)
        notFound();
    }
}
...
export async function generateMetadata({ params }: { params: { id: string } }) {
    ...
    return {
        title: data?.data?.["title"]?.toString() || 'Mv2',
    }
}
