...
import { useBulkDeleteTestaMutation } from '@/app/string-array-example/redux/features/testa/testaSlice'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteTesta, { isLoading }] = useBulkDeleteTestaMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((testa) => testa._id)
            await bulkDeleteTesta({ ids }).unwrap()
