...
export async function getPostbSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        const [totalDocs, last24HoursDocs, monthlyStatsRaw] = await Promise.all([
            Postb.countDocuments({}),
            Postb.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
            Postb.aggregate<AggregationResult>([
                {
                    $group: {
                        _id: {
