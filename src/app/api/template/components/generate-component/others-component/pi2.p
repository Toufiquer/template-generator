...
here is an example of two file  updated generate-add.ts and a older version of generate.edit.ts
...
export const generateAddComponentFile = (inputJsonFile: string): string => {
     ...
     *
     * @param {string} key The schema key, used to create a unique variable name.
     * @param {string | undefined} optionsString The comma-separated string of options from the JSON.
     * @param {Array<{label: string, value: string}>} defaultOptions The default options to use if optionsString is not provided.
     * @returns {string} The name of the generated variable (e.g., "areaOptions").
     */
    const generateOptionsVariable = (
        ...
        // Add the complete variable definition to the set.
        componentBodyStatements.add(
            `const ${varName} = ${optionsJsArrayString};`
        )

        // Return the name for use in the component JSX.
        return varName
    }
    ...
    // --- MODIFICATION: Only add the dynamicVariables block if there are variables to render ---
...
Now Update the generate-edit.tsx file.

and you must generate it with typescript. and look those 4 case 
    1. SELECT
    2. DYNAMICSELECT
    3. RADIOBUTTON
    4. MULTIOPTIONS
