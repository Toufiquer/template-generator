...
import { IPostsa } from '../store/data/data'
import { pageLimitArr } from '../store/store-constant'
import { usePostsaStore } from '../store/store'
import { useGetPostsaQuery } from '@/redux/features/postsa/postsaSlice'
...
const ViewTableNextComponents: React.FC = () => {
    ...
    const handleSort = (key: DisplayablePostsaKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IPostsa) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IPostsa) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedPostsa(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedPostsa(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedPostsa(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Postsa_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
