...
import { postsaSelectorArr } from '../store/store-constant'
import { useBulkUpdatePostsaMutation } from '@/redux/features/postsa/postsaSlice'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdatePostsa, { isLoading }] = useBulkUpdatePostsaMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdatePostsa(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit postsa:', error)
            ...
            <DialogContent>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {postsaSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((posta, idx) => (
                            <div
                                key={(posta._id as string) || idx}
