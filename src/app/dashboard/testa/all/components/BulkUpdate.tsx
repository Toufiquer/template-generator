...
import { testaSelectorArr } from '../store/store-constant'
import { useBulkUpdateTestaMutation } from '@/redux/features/testa/testaSlice'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateTesta, { isLoading }] = useBulkUpdateTestaMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateTesta(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit testa:', error)
            ...
            <DialogContent>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {testaSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((testa, idx) => (
                            <div
                                key={(testa._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(testa)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{testa['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
