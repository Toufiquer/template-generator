...
import { postsbSelectorArr } from '../store/store-constant'
import { useBulkUpdatePostsbMutation } from '@/redux/features/postsb/postsbSlice'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdatePostsb, { isLoading }] = useBulkUpdatePostsbMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdatePostsb(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit postsb:', error)
            ...
            <DialogContent>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {postsbSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((postb, idx) => (
                            <div
                                key={(postb._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(postb)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{postb['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
