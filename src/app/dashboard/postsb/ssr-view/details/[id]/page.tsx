...
const DataDetails = ({ data }: { data: IPostb }) => {
    return (
...
const getDataById = async (id: string): Promise<ApiResponse> => {
    ...
    const backendUrl = `${baseUrl}/generate/postsb/all/api/v1?id=${id}`
    ...
    try {
        ...
        console.error('Failed to fetch Postb:', error)
        notFound();
    }
}
...
export async function generateMetadata({ params }: { params: { id: string } }) {
    ...
    return {
        title: data?.data?.["title"]?.toString() || 'Postb',
    }
}
