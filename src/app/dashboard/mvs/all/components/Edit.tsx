...
import { useUpdateMvsMutation } from '@/redux/features/mvs/mvsSlice.ts'
...
const EditNextComponents: React.FC = () => {
    ...
    const [updateMvs, { isLoading }] = useUpdateMvsMutation()
    const [editedMv, setMv] = useState<IMvs>(defaultMvs)
    ...
    const handleFieldChange = (name: string, value: unknown) => {
        setMv(prev => ({ ...prev, [name]: value }));
    };

    const handleEditMv = async () => {
        if (!selectedMvs) return

        try {
            const { _id, createdAt, updatedAt, ...updateData } = editedMv;
            await updateMvs({
                id: selectedMvs._id,
                ...updateData,
            }).unwrap()
    ...
    return (
        <Dialog open={isEditModalOpen} onOpenChange={toggleEditModal}>
            <DialogContent className="sm:max-w-[625px]">
                <DialogHeader>
                    <DialogTitle>Edit Mv</DialogTitle>
                </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid grid-cols-4 items-center gap-4 pr-1">
                            ...
                            <div className="col-span-3">
                                <InputFieldForString id="title" placeholder="Title" value={editedMv['title']} onChange={(value) => handleFieldChange('title', value as string)} />
                            </div>
                        </div>
                    </div>
                </ScrollArea>
