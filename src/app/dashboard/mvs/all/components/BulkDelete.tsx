...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteMvs, { isLoading }] = useBulkDeleteMvsMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((mvs) => mvs._id)
            await bulkDeleteMvs({ ids }).unwrap()
            toggleBulkDeleteModal(false)
            setBulkData([])
            handleSuccess('Delete Successful')
        } catch (error) {
            console.error('Failed to delete Mvs:', error)
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col">
                        {bulkData.map((mvs, idx) => (
                            <span
                                key={(mvs._id as string) + idx}
                                className="text-xs"
                            >
                                {idx + 1}. {(mvs)['title'] as string || ''}
                            </span>
                        ))}
                    </div>
                </ScrollArea>
