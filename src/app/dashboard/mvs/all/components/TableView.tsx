...
type DisplayableMvsKeys = 
    | 'title'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableMvsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableMvsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.mvs || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableMvsKeys; label: string }[] = [
    ...
    const handleSort = (key: DisplayableMvsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IMvs) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IMvs) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedMvs(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedMvs(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedMvs(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Mvs_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
