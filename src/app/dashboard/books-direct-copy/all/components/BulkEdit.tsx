...
import { booksSelectorArr } from '../store/StoreConstants'
import { useBulkUpdateBooksMutation } from '../redux/rtk-Api'
...
const BulkEditNextComponents: React.FC = () => {
    const { isBulkEditModalOpen, toggleBulkEditModal, bulkData, setBulkData } =
        useBooksStore()
    const [bulkUpdateBooks, { isLoading }] = useBulkUpdateBooksMutation()

    const handleBulkEditBooks = async () => {
        ...
        try {
            ...
            await bulkUpdateBooks(newBulkData).unwrap()
    ...
    const handleRoleChange = (BooksId: string, role: string) => {
        setBulkData(
            bulkData.map((Books) =>
                Books._id === BooksId ? { ...Books, role } : Books
            ) as IBooks[]
        )
    }
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((Books, idx) => (
                            <div
                                key={(Books._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(Books.name as string) || ''}
                                </span>
