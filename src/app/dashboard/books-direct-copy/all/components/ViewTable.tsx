...
const ViewTableNextComponents: React.FC = () => {
    ...
    const getAllBooksData = useMemo(
        () => getResponseData?.data?.books || [],
        [getResponseData]
    )
    ...
    const handleSort = (key: keyof IBooks) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const sortedBooksData = useMemo(() => {
        if (!sortConfig) return getAllBooksData
        return [...getAllBooksData].sort((a, b) => {
    ...
    }, [getAllBooksData, sortConfig])
    const handleSelectAll = (isChecked: boolean) =>
        setBulkData(isChecked ? getAllBooksData : [])
    const handleSelectRow = (isChecked: boolean, Books: IBooks) =>
        setBulkData(
            isChecked
                ? [...bulkData, Books]
                : bulkData.filter((item) => item.email !== Books.email)
        )
    const handlePopUp = () => {
        handleSuccess('Reload Successful')
    }
    const renderActions = (Books: IBooks) => (
    ...
    const renderTableRows = () =>
        sortedBooksData.map((Books: IBooks, index: number) => (
            <TableRow key={(Books.email as string) || index}>
                <TableCell>
                    <Checkbox
                        onCheckedChange={(checked) =>
                            handleSelectRow(!!checked, Books)
                        }
                        checked={bulkData.some(
                            (item) => item.email === Books.email
                        )}
                    />
                </TableCell>
                <TableCell className="font-medium">
                    {(Books.name as string) || ''}
                </TableCell>
                <TableCell>{(Books.email as string) || ''}</TableCell>
                <TableCell>{(Books.passCode as string) || ''}</TableCell>
                <TableCell>{(Books.alias as string) || ''}</TableCell>
                <TableCell>
                    <span
                        className={`py-1 rounded-full text-xs font-medium bg-green-500 text-green-50 px-3`}
                    >
                        {(Books.role as string) || ''}
                    </span>
                </TableCell>
                <TableCell>{formatDate(Books.createdAt)}</TableCell>
                    ...
                    <TableRow>
                        <TableHead>
                            <Checkbox
                                ...
                                checked={
                                    bulkData.length === getAllBooksData.length
                                }
                            />
                        </TableHead>
                        {[
                            ...
                            <TableHead
                                ...
                                onClick={() => handleSort(key as keyof IBooks)}
                            >
