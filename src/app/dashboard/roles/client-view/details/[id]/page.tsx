...
const Page = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const url = `${baseUrl}/generate/roles/all/api/v1?id=${id}`;
    ...
    if (loading) {
        return <div className="text-center p-8">Loading Role details...</div>
    }
    ...
    if (!data) {
        return <div className="text-center p-8">No data available for this Role.</div>
    }
    ...
    return (
        <div className="w-full flex flex-col md:p-4 p-1 gap-4">
            ...
            <div className="w-full hover:bg-slate-200 bg-slate-100 block p-2 border-b border-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 dark:border-slate-500">
                <strong className="capitalize">role:</strong> <pre className="text-sm bg-slate-200 dark:bg-slate-900 p-2 rounded-md whitespace-pre-wrap">{JSON.stringify(data?.["role"], null, 2)}</pre>
            </div>
            </div>
            <Link
