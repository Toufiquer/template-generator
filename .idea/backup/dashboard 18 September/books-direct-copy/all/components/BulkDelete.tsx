...
import { useBulkDeleteBooksMutation } from '../redux/rtk-Api'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteBooks, { isLoading }] = useBulkDeleteBooksMutation()

    const handleBulkDeleteNextComponents = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((Books) => Books._id)
            await bulkDeleteBooks({ ids }).unwrap()
