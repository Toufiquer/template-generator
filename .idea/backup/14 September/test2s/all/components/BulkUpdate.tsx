...
import { test2sSelectorArr } from '../store/store-constant'
                ...
                {bulkData.length > 0 && (
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            <p>Set all <span className="font-semibold">Area</span> to</p>
                            <Select
                                onValueChange={(value) =>
                                    handleFieldChangeForAll(value)
                                }
                                defaultValue={
                                    (test2sSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {test2sSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                    ...
                    <div className="flex flex-col gap-2">
                        {bulkData.map((test2, idx) => (
                            ...
                            >
                                ...
                                <span className="text-blue-500">{test2['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
