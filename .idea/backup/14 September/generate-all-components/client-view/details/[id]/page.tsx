...
const Page = () => {
    ...
    useEffect(() => {
        if (id) {
            const fetchData = async () => {
                ...
                const url = `http://localhost:3000/dashboard/test2/all/api/v1?id=${id}`
    ...
    return (
        <div className="w-full flex flex-col md:p-4 p-1 gap-4">
            ...
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
                <strong className="capitalize">books list:</strong> {(data?.["books-list"] as string[])?.join(', ')}
            </div>
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
                <strong className="capitalize">check list:</strong> {(data?.["check-list"] as string[])?.join(', ')}
            </div>
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
