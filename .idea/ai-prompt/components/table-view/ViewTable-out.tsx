...
const ViewTableNextComponents: React.FC = () => {
    ...
    const handleSelectAll = (isChecked: boolean) =>
        setBulkData(isChecked ? getAllPostsData : [])
    const handleSelectRow = (isChecked: boolean, Posts: IPosts) =>
        setBulkData(
            isChecked
                ? [...bulkData, Posts]
                : bulkData.filter((item) => item.email !== Posts.email)
        )
    const handlePopUp = () => {
    ...
    const renderTableRows = () =>
        sortedPostsData.map((Posts: IPosts, index: number) => (
            <TableRow key={(Posts.email as string) || index}>
                <TableCell>
                    <Checkbox
                        onCheckedChange={(checked) =>
                            handleSelectRow(!!checked, Posts)
                        }
                        checked={bulkData.some(
                            (item) => item.email === Posts.email
                        )}
                    />
                </TableCell>
                <TableCell className="font-medium">
                    {(Posts.name as string) || ''}
                </TableCell>
                <TableCell>{(Posts.email as string) || ''}</TableCell>
                <TableCell>{(Posts.passCode as string) || ''}</TableCell>
                <TableCell>{(Posts.alias as string) || ''}</TableCell>
                <TableCell>
                    <span
                ...
                <TableCell>{formatDate(Posts.createdAt)}</TableCell>
                    ...
                    <TableRow>
                        <TableHead>
                            <Checkbox
                                ...
                                checked={
                                    bulkData.length === getAllPostsData.length
                                }
                            />
                        </TableHead>
                        {[
